...
def verify_claim(case: CaseInput, quality: Member1Output, visual: Member2Output,
    ...
    verdict, score, reason = "ambiguous", None, "unsupported_or_missing_claim"
    if not case.image_paths or not case.claim_text.strip():
    ...
    elif quality.claimed_location or quality.product:
        reason = "product_or_location_not_verified"
        ...
        if visual.damage_type == "no_damage":
            ...
            verdict, score, reason = "negative", 0.0, "damage_type_mismatch"
    ...
    return VerificationResult(member2=Member2Output.model_validate(payload),
                              verdict=verdict, consistency_score=score, reason_code=reason)
